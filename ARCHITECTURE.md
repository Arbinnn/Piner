...
| **`rewritePositionalPlotArgs()`** — names `plot()`'s positional arguments from `linewidth` on | `outputCall` builds a plot's options out of NAMED arguments only, reading just `series`, `title` and `color` positionally. `plot(hist, 'Histogram', css, 1, plot.style_columns)` — how nearly every published MACD writes it — arrived with `options: {}`, so the style was gone and the histogram drew as a line, silently. Naming them is a no-op on TradingView and restores `style`, `histbase`, `offset`, `display` and the rest. Only the affected call's columns shift, and only right of the insertion. | keep |
...
| Fix | Symptom it cures | TV |
|---|---|---|
| **`COLUMN_STYLES`** → a `HistogramSeries` for `plot.style_columns` / `style_histogram` | Every plot was mounted as a `LineSeries`, so a MACD histogram drew as a zig-zag line through zero, indistinguishable from the two lines sharing its pane. A style change now recreates the series (`TrackedPlot.columns`) rather than re-optioning one of the wrong type, and `histbase` becomes the bar's `base`. Depends on §7.1's `rewritePositionalPlotArgs()` — without it a positionally-styled plot never reports a style at all. | port |
...
| **Empty plots rejected as pane host** (`hasData`) | The rung below the whitespace bug, and far commoner: a script that gates its only plot behind an input — `plot(show ? value : na)` — leaves a *visible series with zero points*. It was preferred over seeding a host, joined no price scale, and silently killed every drawing on the pane. Now the host falls through to `seedPaneHost` with the drawings' own extent. | moot |
...
| Fix | Symptom it cures | Where | TV |
|---|---|---|---|
| **`linefill` drawn** (`linefillPolygon()`, own `bottom` pane view) | The drawing renderer handled box/line/label/polyline and dropped `linefill` on the floor — silently, since it is a valid object that simply never reached a `case`. Channel and range scripts put their entire bias shading in one linefill: LuxAlgo's Range Sentiment Profile drew all its levels and bins with no band between them. A linefill carries only the **ids** of its two lines, so it resolves against the frame's line objects; it paints even when those lines are `color = na`, which is how "show the band but not its edges" is written. Its own view sorts under boxes and lines regardless of creation order. | [drawings.ts](src/lib/drawings.ts) | port |

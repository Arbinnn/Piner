/**
 * The Pine backend's request handler.
 *
 * Two endpoints, no framework — `node:http` types are enough for a JSON API this size.
 *
 *   GET  /api/candles?symbol=&timeframe=&bars=   OHLCV for the chart
 *   POST /api/pine/execute                       compile + run a script, return plot data
 *
 * Exported as a plain `(req, res)` handler so it can be driven by `node:http` locally
 * (`server/index.ts`) or wrapped as a Vercel serverless function (`api/[...path].ts`) — both
 * pass compatible request/response objects.
 */
...
export function handleRequest(req: IncomingMessage, res: ServerResponse): Promise<void> {
  return route(req, res).catch((err: unknown) => {
    if (res.headersSent) {
      res.end();
      return;
    }
